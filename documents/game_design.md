...
### How Everything Works Together | 系统之间的关系

想象你是一名考古学家，准备去探险：

#### 1️⃣ **Level (等级) = Your Experience | 你的经验**

- **你刚开始**（Level 1）：只能带 **3 件装备**
- **随着经验积累**（Level 3）：可以带 **4 件装备**  
- **成为专家**（Level 7）：可以带 **8 件装备**（最多！）

💡 **等级越高 = 背包越大 = 能带更强的装备组合**

#### 2️⃣ **Items (物品) = Your Tools | 你的工具**
...
#### 3️⃣ **Total Weight (总权重) = Your Power | 你的实力**

把你选的所有装备加起来：

- **轻装探索**（1-20分）：比如带 Camera + Shovel + Brush = 4分
- **中等配置**（15-50分）：比如带 Drone + Magnetometry + Trowel = 13分
- **专业级别**（40分以上）：比如带 LiDAR + GPR + 其他装备 = 60分

💡 **总分越高 = 探索能力越强**

#### 4️⃣ **Routes (路线) = Exploration Method | 探索方法**

根据你的总分，系统自动匹配探索方法：

| 你的总分 | 匹配到的路线 | 探索方式 |
|---------|------------|----------|
| **1-20分** | Beginner Route | 地表勘测（找容易的遗迹）|
| **15-50分** | Advanced Route | 中度挖掘（找中等难度遗迹）|
| **40分以上** | Master Route | 深度探索（找困难和隐藏遗迹）|

💡 **你的装备越好 = 解锁更高级的探索方法**

#### 5️⃣ **Ruins (遗迹) = What You Can Find | 你能发现什么**
...
```
1. 你有 6 件基础装备（Camera, Shovel, Hand Pick, Brush, Dental Pick, Bucket）
2. 背包限制：只能带 3 件
3. 你选择：Camera + Shovel + Hand Pick = 总分 4
4. 系统匹配：Beginner Route（地表勘测）
5. 可发现：金字塔、兵马俑、长城等简单遗迹
6. 发现成功！获得 40-70 经验值
7. 重复探索，积累经验升级
```
...
```
1. 解锁了传奇装备（LiDAR 和 GPR）
2. 背包升级：可以带 6 件装备
3. 你选择：LiDAR + GPR + Drone + Magnetometry + Trowel + Camera = 总分 34
4. 系统匹配：Advanced Route（进阶探索）
5. 可发现：所有新手+进阶遗迹，如卡纳克神庙、紫禁城
6. 如果加装备至 40+ 分，可触发 Master Route
7. 发现隐藏遗迹！获得 80+ 经验值
8. 继续冲击 Level 10
```
...
升级！(Level Up)
   ↓
解锁新装备 + 扩大背包 (New Items + More Slots)
```
...
- 没有"必须用这个装备"的限制
- 只要总分够，任何组合都可以
- 你可以带 3 件传奇装备（30分），也可以带 6 件基础装备（18分）

#### ✅ 平滑进度

- 路线权重范围有**重叠**
- 比如：Beginner (1-20) 和 Advanced (15-50) 在 15-20 分重叠
- 让你从简单过渡到困难时不会突然卡住
...
- **Common（普通）**：浅奶油色卡片 - 基础工具
- **Rare（稀有）**：中等褐色卡片 - 高级设备  
- **Legendary（传奇）**：深咖啡色卡片 - 顶级科技

所有颜色统一使用**大地色系**，营造考古探险的复古氛围。
